...
| **gtm/company_people.py** | Paint a company's public **people picture**: `contact_evidence` + `recipes.people` → roster (name + title from schema.org **and** plain-HTML team pages) + how to reach in + an honest coverage note when a site has no listed team. | `python examples/gtm/company_people.py https://gitlab.com` |
...
Or get a whole company in one call — contacts, decision-makers, and firmographics, all sourced:

```python
from pith.cli import contact_evidence
from pith.recipes import owner_email, people
ev = contact_evidence("https://arizonabiltmoredentistry.com")   # crawls team/about/contact, folds every source
owner_email(ev["facts"]); people(ev["facts"]); ev["firmographics"]   # email · decision-makers · rating/hours/geo
```
